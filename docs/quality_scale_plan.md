...
- ✅ Ratchet in place: `[tool.mypy]` (strict) in `pyproject.toml`, `mypy_baseline.json` with a per-module ceiling, `scripts/typing_ratchet.py` (fails on any regression, `--update` only lowers), workflow `strict-typing.yml`. 15 of 28 modules are clean (`const`, `data`, `gateway`, `myhome_device`, `services`, `device_trigger`, `decoder_pool`, transports, diagnostics, repairs, bus monitor); **550 errors remain in 13 modules** — biggest first: `config_flow` 105, `cover` 66, `validate` 57, `sensor` 56, `binary_sensor` 44, `light` 41, `climate` 31, `websocket` 30, `button` 29, `__init__` 27, `alarm_control_panel` 21, `button` 20, `switch` 18. Each PR that touches a module should leave it lower and run `--update`.
